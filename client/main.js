...
function initializeGraph() {
  ...
  const node = svg
    ...
    .attr("r", 20)
    .attr("fill", (d) =>
      selectedNode && selectedNode.id === d.id ? "#fabb54ff" : "#f2cc8f"
    )
    .call(
      d3
        .drag()
...
function handleNodeClick(event, d) {
  ...
  d3.selectAll(".node").attr("fill", (node) =>
    node.id === d.id ? "#fabb54ff" : "#f2cc8f"
  );
